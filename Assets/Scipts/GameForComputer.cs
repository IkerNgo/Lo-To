...
public class GameForComputer : MonoBehaviour
{
    ...
    private void newGame()
    {
        ...
        generateColor();
        board.draw(state, colorNumber);                      //Hien thi bàn game
        Vector3 m = board.getTilePositionForComputer(12, 12);
        Instantiate(frame, board.getTilePositionForComputer(12, 12), Quaternion.identity);
    }
        ...
        for (int i = 0; i < 9; i++)
        {
            for (int j = 0; j < 9; j++)
            {
                ...
                if (numberArray[i, j] != 0)        //Neu khác 0 se tien hành gán so và vi trí vào Cell
                {
                    ...
                    cell.had = false;
                    Vector3 position = board.getTilePositionForComputer(j, 8 - i);//Lay vi trí cua ô theo toa do the gioi
    ...
    private void getHadCell()
    {
        ...
        for(int i=0; i<=8; i++)
        {
            for(int j=0; j<=8; j++)
            {
                if (state[8-i,j].type==Cell.Type.Number)
                {
                    if (gameManager.numberCalledList.Contains(state[8-i,j].number) && !state[8-i,j].had)
                    {
                        Instantiate(hadCell, board.getTilePositionForComputer(j+8, i+8), Quaternion.identity);
                        state[8-i,j].had = true;
                    }
                }
            }
        }
